...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    expected_values = mdp_data['value'].dot(mdp_data['transition_probs'][state, :, :])
    if expected_values[0] > expected_values[1]:
        return 0
    elif expected_values[1] > expected_values[0]:
        return 1
    else:
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    total_count = np.sum(mdp_data['transition_counts'], axis=1)
    for i in range(mdp_data['num_states']):
        for j in range(2):
            if total_count[i, j] != 0:
                mdp_data['transition_probs'][i, :, j] = mdp_data['transition_counts'][i, :, j] / total_count[i, j]
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    # *** START CODE HERE ***
    updated_value = gamma * np.max(mdp_data['value'].dot(mdp_data['transition_probs']), axis=1) + mdp_data['reward']
    if np.max(np.abs(updated_value - mdp_data['value'])) < tolerance:
        mdp_data['value'] = updated_value
        return True
    while np.max(np.abs(updated_value - mdp_data['value'])) >= tolerance:
        mdp_data['value'] = updated_value
        updated_value = gamma * np.max(mdp_data['value'].dot(mdp_data['transition_probs']), axis=1) + mdp_data['reward']
    mdp_data['value'] = updated_value
    return False
