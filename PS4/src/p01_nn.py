...
def get_initial_params():
    ...
    return {
        'W1': np.random.normal(size=(CONVOLUTION_FILTERS, 1, CONVOLUTION_SIZE, CONVOLUTION_SIZE),
        ...
        for y in range(input_height - conv_height + 1):
            for output_channel in range(conv_channels):
                output[output_channel, x, y] = np.sum(
                    np.multiply(data[:, x:(x + conv_width), y:(y + conv_height)], conv_W[output_channel, :, :, :])) + \
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    for i in range(conv_width):
        for j in range(conv_height):
            for k in range(conv_channels):
                grad_weights[k, :, i, j] = np.sum(
                    data[:, i: i + input_width - conv_width + 1, j: j + input_height - conv_height + 1] * output_grad[k, :, :],
                    axis=(1, 2))

    for x in range(input_width - conv_width + 1):
        for y in range(input_height - conv_height + 1):
            grad_data[:, x: x + conv_width, y: y + conv_height] += np.sum(
                output_grad[:, x, y].reshape((-1, 1, 1, 1)) * conv_W[:, :, :], axis=0)

    return grad_weights, np.sum(output_grad, axis=(1, 2)), grad_data
    # *** END CODE HERE ***
...
def forward_max_pool(data, pool_width, pool_height):
    ...
    for x in range(0, input_width, pool_width):
        for y in range(0, input_height, pool_height):
            output[:, x // pool_width, y // pool_height] = np.amax(data[:, x:(x + pool_width), y:(y + pool_height)],
                                                                   axis=(1, 2))
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    for x in range(0, input_width, pool_width):
        for y in range(0, input_height, pool_height):
            for z in range(input_channels):
                grad_data[z, x: x + pool_width, y: y + pool_height][z, np.unravel_index(
                    data[z, x: x + pool_width, y: y + pool_height].argmax(), (pool_width, pool_height))] = output_grad[z, x // pool_width, y // pool_width]
...
def backward_prop(data, labels, params):
    ...
    grad_W2, grad_b2, grad_linear_data = backward_linear(W2, b2, flattened, grad_softmax)
    grad_relu = backward_relu(first_max_pool, grad_linear_data.reshape(first_max_pool.shape))
    grad_max_pool = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, grad_relu)
    grad_W1, grad_b1, grad_convolution_data = backward_convolution(W1, b1, data, grad_max_pool)
